...
echo "🔧 PASSO 2: Corrigindo Endpoint de Upload"
echo "=========================================="
echo ""

# Criar o endpoint correto de upload
cat > app/api/v1/upload_fix.py << 'EOF'
...
echo "✅ Arquivo upload_fix.py criado!"
echo ""

# Atualizar main.py para incluir o novo router
...
if 'upload_fix' not in content:
    # Adicionar import
    if 'from app.api.v1 import' in content:
        content = content.replace(
            'from app.api.v1 import auth, organizations, members, signature_templates, signatures, banners, analytics, uploads',
            'from app.api.v1 import auth, organizations, members, signature_templates, signatures, banners, analytics, uploads, upload_fix'
        )
    
    # Adicionar router
    if 'app.include_router(uploads.router' in content:
        content = content.replace(
...
app.include_router(upload_fix.router, prefix=f"{settings.API_V1_STR}/upload", tags=["upload"])'''
    ...
    print("ℹ️ main.py já está atualizado")
PYTHON_EOF
...
echo "   bash 02_corrigir_upload.sh"
